...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.laptop-card').forEach(card => {
    card.addEventListener('click', (event) => {
      // Jika yang diklik adalah tombol beli, tombol favorit, atau link, biarkan aksi aslinya
      if (event.target.closest('.buy-button') || event.target.closest('.product-fav-btn') || event.target.closest('a')) return;
